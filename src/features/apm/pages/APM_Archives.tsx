...
import { createAPMArchiveColumns } from '../../../shared/services/table-columns/archive-columns';
...
const APMArchives: React.FC<APMArchivesProps> = ({ projectNotes = [] }) => {
  ...
  const loadArchivedBids = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      // Get all bids and filter for APM archived bids client-side
      const data = await dbOperations.getBids();
      
      // Filter for projects sent to APM that are archived by APM (apm_archived = true)
      const apmArchivedBids = data.filter((bid: Bid) => 
        bid.sent_to_apm === true && bid.apm_archived === true
      );
      
      // Transform the data to match our Bid interface
      const transformedBids = apmArchivedBids.map((bid: Record<string, unknown>) => ({
        ...bid,
      ...
      console.error('Error loading APM archived bids:', err);
      setError(err instanceof Error ? err.message : 'Failed to load archived APM projects');
      showError('Load Failed', 'Failed to load archived APM projects. Please refresh the page.');
    } finally {
      setLoading(false);
    }
  }, [showError]);
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Error Loading Archives</h3>
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">Archived projects will appear here</p>
              <button
              ...
              <DataTable
                ...
                emptyMessage={
                  searchTerm || statusFilter.length > 0 || dateRange.startDate || dateRange.endDate
                    ? 'No matching archived APM projects'
                    : 'No archived APM projects'
                }
