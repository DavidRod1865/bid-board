...
const Folder: React.FC<FolderProps> = ({ 
  ...
  // Sync open state with isActive or isHovered
  useEffect(() => {
    setOpen(isActive || isHovered);
  }, [isActive, isHovered]);

  const folderBackColor = darkenColor(color, 0.08);
  const paper1 = darkenColor('#fbbf24', 0.2);
  const paper2 = darkenColor('#fbbf24', 0.1);
  const paper3 = '#fbbf24';

  const handleClick = () => {
    // Only allow manual toggling if not controlled by isActive or isHovered
