...
export const isActiveStatus = (status: string): boolean => {
  const active = ['new', 'gathering costs', 'pending pricing', 'sent to gc', 'on-hold'];
...
export const getStatusDescription = (status: string): string => {
  const descriptions: Record<string, string> = {
    ...
    'pending pricing': 'Finalizing pricing and preparing bid documentation',
    'sent to gc': 'Proposal submitted to general contractor, awaiting response',
