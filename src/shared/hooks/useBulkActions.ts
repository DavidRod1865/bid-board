...
export type BulkActionType = 'moveToActive' | 'archive' | 'onHold' | 'delete' | 'apmMoveToActive' | 'apmArchive' | 'apmOnHold' | 'unsendFromAPM' | 'sendToAPM';
    ...
    try {
      ...
      switch (actionType) {
        ...
        case 'delete':
          ...
          break;
        case 'apmMoveToActive':
          result = await bulkOperations.apmBulkMoveToActive(bidIds);
          break;
        case 'apmArchive':
          result = await bulkOperations.apmBulkArchive(bidIds);
          break;
        case 'apmOnHold':
          result = await bulkOperations.apmBulkOnHold(bidIds);
          break;
        case 'unsendFromAPM':
          result = await bulkOperations.bulkUnsendFromAPM(bidIds);
          break;
        case 'sendToAPM':
          result = await bulkOperations.bulkSendToAPM(bidIds);
          break;
        default:
      ...
      if (result.success) {
        const actionLabels = {
          ...
          delete: 'deleted',
          apmMoveToActive: 'moved to active',
          apmArchive: 'archived',
          apmOnHold: 'moved to on-hold',
          unsendFromAPM: 'removed from APM',
          sendToAPM: 'sent to APM'
        };
...
export const getBulkActionConfirmationMessage = (actionType: BulkActionType, count: number): string => {
  ...
  switch (actionType) {
    ...
    case 'unsendFromAPM':
      return `Are you sure you want to remove ${count} ${projectText} from APM? This will return them to Estimating-only status.`;
    case 'sendToAPM':
      return `Are you sure you want to send ${count} ${projectText} to the APM team for management?`;
    case 'delete':
